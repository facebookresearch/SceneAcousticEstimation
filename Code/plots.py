...
def plot_waveform(waveform: torch.Tensor, sample_rate: int, title: str = "Waveform",
                  xlim: List = None, ylim: List = None, ax=None):
...
def plot_stft(stft, title=None, figsize=(6, 4), axs=None, vmin=-100, log=True, force_log=False):
    """ Plots a single STFT.
    Supports real and complex values.
    For complex, we plot the magnitude and phase."""
    assert len(stft.shape) == 3, 'The signal should be [channels, freqs, frames] for plotting.'
    assert stft.shape[0] == 1, "Only single channel is supported"
    if torch.all(torch.isreal(stft)) and torch.any(stft > 0):  # check for normalized values
        warnings.warn("WARNING: Some values of the STFT are larger than 0, and will be clipped in the plot.")

    if axs is None:
        if torch.all(torch.isreal(stft)):
    ...
    if log:
        ...
        if torch.all(torch.isreal(stft)):
            ...
            vmin, vmax = stft.abs().min(), stft.abs().max()
    if torch.all(torch.isreal(stft)):
        aa = ax.matshow(stft[0, :, :], aspect='auto', origin='lower', cmap='magma', vmin=vmin, vmax=vmax)
        fig.colorbar(aa, ax=ax, location='right')
        ax.grid(False)
    else:
        if force_log:
            div = stft.abs().max()
            stft_mag = 20 * torch.log10((stft.abs() + 1e-10) / div)  # got to db
        else:
            stft_mag = stft.abs()

        if torch.any(stft_mag[0, :, :].abs() < vmin) or torch.any(stft_mag[0, :, :].abs() > vmax):
            warnings.warn("WARNING: Some values of the STFT are larger than 0, and will be clipped in the plot.")
        aa = ax[0].matshow(stft_mag[0, :, :], aspect='auto', origin='lower', cmap='magma', vmin=vmin, vmax=vmax)
        fig.colorbar(aa, ax=ax[0], location='right')
        ax[0].grid(False)

        aa = ax[1].matshow(stft[0, :, :].angle(), aspect='auto', origin='lower', cmap='magma')
...
def plot_matrix2polar(t_matrix, order, plot_channel=0, linewidth=2, INDB=True, rlim=None, title=None, axs : List = None,
    ...
    Y_grid = spa.sph.sh_matrix(order, azis, utils.ele2colat(np.zeros_like(azis)), sh_type='real')  # Level seems ok for input_order == 1
    ...
    Y_grid = spa.sph.sh_matrix(order, eles_azis, utils.ele2colat(eles), sh_type='real')
    Y_grid = torch.from_numpy(Y_grid) * scale
...
def plot_matrix2polarLinear(t_matrix, order, method='fibonacci', plot_channel=0, INDB=True, rlim=None, title=None,
    ...
    Y_grid = spa.sph.sh_matrix(order, sphere_dirs[:, 0], sphere_dirs[:, 1], sh_type='real')
